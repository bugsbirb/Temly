...
export const MultiSelect = React.forwardRef<MultiSelectRef, MultiSelectProps>(
		(
			...
			const selectedValues = value !== undefined ? value : uncontrolledSelectedValues;
		...
		const handleInputKeyDown = (
			...
			if (event.key === "Enter") {
				setIsPopoverOpen(true);
						...
						if (value === undefined) {
							setUncontrolledSelectedValues(newSelectedValues);
						}
		...
		const toggleAll = () => {
			...
			const allOptions = getAllOptions().filter((option) => !option.disabled);
					if (selectedValues.length === allOptions.length) {
						...
						if (value === undefined) {
							setUncontrolledSelectedValues(allValues);
						}
		...
		React.useEffect(() => {
					...
					if (!arraysEqual(prevDefaultValue, defaultValue)) {
						if (value === undefined && !arraysEqual(uncontrolledSelectedValues, defaultValue)) {
							setUncontrolledSelectedValues(defaultValue);
						}
						prevDefaultValueRef.current = [...defaultValue];
					}
				}, [defaultValue, uncontrolledSelectedValues, arraysEqual, resetOnDefaultValueChange, value]);
				...
				<Popover
					...
					<PopoverTrigger asChild>
						<Button
							...
							className={cn(
								"flex p-1 rounded-lg border min-h-11 h-auto items-center justify-between bg-inherit hover:bg-inherit [&_svg]:pointer-events-auto",
